...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        self.v_t = [torch.zeros_like(p) for p, dp in self.params]
        # ========================
    ...
    def step(self):
        ...
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            v_t1 = self.momentum * self.v_t[i] - self.learn_rate * dp
            self.v_t[i] = v_t1
            p += v_t1
            i+=1
            dp.zero_()
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # TODO: Add your own initializations as needed.
        # ====== YOUR CODE: ======
        self.r_t = [torch.zeros_like(p) for p, dp in self.params]
        # ========================
    ...
    def step(self):
        ...
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            r_t1 = self.decay * self.r_t[i] + (1 - self.decay) * torch.pow(dp, 2)
            self.r_t[i] = r_t1
