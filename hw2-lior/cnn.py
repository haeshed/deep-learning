...
class CNN(nn.Module):
    ...
    def _make_mlp(self):
        ...
        hidden_dims.append(self.out_classes)
        nonlins = [ACTIVATIONS[self.activation_type](**self.activation_params)] * len(hidden_dims)
        mlp = MLP(in_dim, hidden_dims, nonlins)
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        channels.insert(0, in_channels)
        for i in range(len(channels) - 1):
            layers_main.append(nn.Conv2d(channels[i], channels[i + 1], kernel_sizes[i], padding=kernel_sizes[i] // 2))
            if i < len(channels) - 2:
                if dropout != 0:
                    layers_main.append(nn.Dropout2d(dropout))
                if batchnorm:
                    layers_main.append(nn.BatchNorm2d(channels[i + 1]))
        ...
        if in_channels != channels[-1]:
            ...
            layers_shortcut.append(nn.Identity())
...
class ResNet(CNN):
    ...
    def _make_feature_extractor(self):
        ...
        for i in range(0, N, P):
            block_channels = channels[i:i+P+1]
            if self.bottleneck and in_channels == block_channels[-1]:
                inner_channels = block_channels[1:-1]
                inner_kernel_sizes = [3] * len(inner_channels)  # Using 3x3 convolutions
                block = ResidualBottleneckBlock(
                    in_out_channels=block_channels[0],
                ...
                kernel_sizes = [3] * (len(block_channels) - 1)  # Using 3x3 convolutions
                block = ResidualBlock(
