...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = torch.zeros_like(x)  # Initialize dx with the same shape as x
        dx[x > 0] = 1     # The derivative is 1 where x is positive
        dx[x <= 0] = self.alpha  # The derivative is alpha where x is non-positive
        dx *= dout
        # ========================
...
class Sigmoid(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        out = 1 / (1 + torch.exp(-x))
        self.grad_cache['x'] = out  # Cache the output for use in backward pass
        # ========================
    ...
    def backward(self, dout):
        ...
        # TODO: Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        sigmoid_out = self.grad_cache['x']
        dx = dout * sigmoid_out * (1 - sigmoid_out) # sigmoid(x) * (1 - sigmoid(x))
...
class TanH(Layer):
    ...
    def backward(self, dout):
        ...
        # TODO: Implement gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        tanh_out = self.grad_cache['x']
        dx = dout * (1 - tanh_out.pow(2))
        # ========================
...
class Linear(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = torch.mm(dout, self.w)  # Gradient w.r.t. input
        self.dw += torch.mm(dout.t(), x)  # Gradient w.r.t. weights
        self.db += dout.sum(0)  # Gradient w.r.t. biases, sum over batch
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        correct_class_scores = x.gather(1, y.view(-1, 1))
        loss = -correct_class_scores + log_sum_exp
        loss = loss.mean()
    ...
    def backward(self, dout=1.0):
        ...
        exps = torch.exp(x)
        softmax = exps / exps.sum(dim=1, keepdim=True)
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        # ====== YOUR CODE: ======
        # Prepare the layer sizes for easy iteration
        layer_sizes = [in_features] + list(hidden_features) + [num_classes]

        # Iterate through the layers and add them to the layer list
        for i in range(len(layer_sizes) - 1):
            # Add the fully connected layer
            layers.append(Linear(layer_sizes[i], layer_sizes[i+1]))

            # Check if the current layer is not the last Linear layer
            if i < len(layer_sizes) - 2:
                ...
                # Add dropout layer if dropout rate is specified and greater than zero
                if dropout > 0:
