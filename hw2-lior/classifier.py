...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        preds = torch.zeros(y_proba.shape[0], dtype=torch.int)
        preds[y_proba[:, self.positive_class] >= self.threshold] = 1
        return preds
        # ========================
...
def plot_decision_boundary_2d(
    ...
    # ====== YOUR CODE: ======
    xs = torch.arange(x[:, 0].min() - 0.5, x[:, 0].max() + 0.5, dx)
    ys = torch.arange(x[:, 1].min() - 0.5, x[:, 1].max() + 0.5, dx)
    x1_grid, x2_grid = torch.meshgrid(xs, ys, indexing='xy')
    grid_points = torch.cat([x1_grid.reshape(-1, 1), x2_grid.reshape(-1, 1)], dim=1)
...
def select_roc_thresh(
    ...
    # ====== YOUR CODE: ======
    preds = classifier.predict_proba(x).detach().numpy()[:, 1]
    fpr, tpr, thresh = roc_curve(y, preds)
    distances = torch.sqrt(torch.from_numpy((fpr ** 2) + (1 - tpr) ** 2))
    optimal_thresh_idx = torch.argmin(distances)
