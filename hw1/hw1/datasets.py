...
def random_labelled_image(
    ...
    """
    image = torch.randint(low=low, high=high, size=shape, dtype=dtype, device=torch.device('cpu'))
    label = torch.randint(low=0, high=num_classes, size=(1,), dtype=dtype, device=torch.device('cpu')).item()
    return image, label
